...
function basename(path2, ext) {
  ...
  while (index--) {
    if (path2.codePointAt(index) === 47) {
      ...
      if (extIndex > -1) {
        if (path2.codePointAt(index) === ext.codePointAt(extIndex--)) {
          if (extIndex < 0) {
            end = index;
          }
        } else {
...
function extname(path2) {
  ...
  let unmatchedSlash;
  while (index--) {
    const code = path2.codePointAt(index);
    if (code === 47) {
      if (unmatchedSlash) {
        startPart = index + 1;
        break;
